...
(function () {
  ...
  function renderCouponModal() {
    ...
    const isSpecific = cp.scope === 'specific';
    const applicable = Array.isArray(cp.applicable_tools) ? cp.applicable_tools : [];

    return `
      <div id="couponModalOverlay" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
        <div class="w-full max-w-lg glass-card rounded-3xl border border-zinc-700/80 shadow-2xl p-6 space-y-5 my-8">
          <div class="flex items-center justify-between border-b border-zinc-800 pb-4">
            <div>
              <h3 class="text-lg font-bold font-syne text-zinc-100">${cp.id ? 'Edit Promo Coupon' : 'Create New Promo Coupon'}</h3>
              <p class="text-xs text-zinc-400 mt-0.5">Configure discount percentage and tool applicability</p>
            </div>
            <button onclick="window.closeCouponModal()" class="w-8 h-8 rounded-full bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-zinc-100 flex items-center justify-center text-sm font-bold transition-colors">✕</button>
          </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <input type="text" id="couponCodeInput" value="${cp.code || ''}" placeholder="e.g. SPECIAL50" class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-3 text-sm text-amber-400 font-mono font-bold uppercase tracking-wider focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors">
              <p class="text-[11px] text-zinc-500 mt-1">Customers enter this promo code at checkout on the website.</p>
            </div>

            <!-- Discount Value Input -->
            <div>
              <label class="block text-xs font-semibold text-zinc-300 mb-1">Discount Percentage (% Off) *</label>
              <div class="relative">
                <input type="number" id="couponDiscountInput" min="1" max="100" value="${cp.discount_value || 20}" placeholder="20" class="w-full bg-zinc-900 border border-zinc-700 rounded-xl px-4 py-2.5 text-sm text-zinc-100 font-bold focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors">
                <span class="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-bold text-amber-400">% OFF</span>
              </div>
            </div>

            <!-- Scope Choice (All Tools vs Specific Tools) -->
            <div>
              <label class="block text-xs font-semibold text-zinc-300 mb-2">Discount Applicability Scope</label>
              <div class="grid grid-cols-2 gap-3">
                <label id="scopeOptionAll" class="flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all ${!isSpecific ? 'border-amber-500/60 bg-amber-500/10 text-amber-300 shadow-sm shadow-amber-500/10' : 'border-zinc-800 bg-zinc-900/60 text-zinc-400 hover:border-zinc-700'}">
                  <input type="radio" name="couponScopeRadio" value="all" ${!isSpecific ? 'checked' : ''} onchange="window.setCouponScope('all')" class="text-amber-500 focus:ring-0">
                  <div class="min-w-0">
                    <div class="text-xs font-bold text-zinc-200">🌐 All Tools</div>
                    <div class="text-[10px] text-zinc-400 truncate">Apply store-wide to all tools</div>
                  </div>
                </label>
                <label id="scopeOptionSpecific" class="flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all ${isSpecific ? 'border-amber-500/60 bg-amber-500/10 text-amber-300 shadow-sm shadow-amber-500/10' : 'border-zinc-800 bg-zinc-900/60 text-zinc-400 hover:border-zinc-700'}">
                  <input type="radio" name="couponScopeRadio" value="specific" ${isSpecific ? 'checked' : ''} onchange="window.setCouponScope('specific')" class="text-amber-500 focus:ring-0">
                  <div class="min-w-0">
                    <div class="text-xs font-bold text-zinc-200">🎯 Specific Tools</div>
                    <div class="text-[10px] text-zinc-400 truncate">Select only specific tools</div>
                  </div>
                </label>
              </div>
            </div>

            <!-- Specific Tools Container (Shown only when scope === 'specific') -->
            <div id="couponToolsSection" style="display: ${isSpecific ? 'block' : 'none'};" class="space-y-2 pt-1">
              <div class="flex items-center justify-between">
                <label class="text-xs font-semibold text-zinc-300">
                  Select Applicable Tools (<span id="couponSelectedToolsCount" class="text-amber-400 font-bold">${applicable.length}</span> selected)
                </label>
                <div class="flex items-center gap-2">
                  <button type="button" onclick="window.couponSelectAllTools(true)" class="text-[11px] text-amber-400 hover:underline">Select All</button>
                  <span class="text-zinc-600">•</span>
                  <button type="button" onclick="window.couponSelectAllTools(false)" class="text-[11px] text-zinc-400 hover:underline">Clear All</button>
                </div>
              </div>

              <!-- Search Filter -->
              <input type="text" id="couponToolSearchInput" oninput="window.filterCouponToolsList(this.value)" placeholder="Filter tools by name..." class="w-full bg-zinc-900 border border-zinc-800 rounded-xl px-3 py-2 text-xs text-zinc-100 placeholder-zinc-500 focus:outline-none focus:border-amber-500">

              <!-- Tool List -->
              <div id="couponToolsListContainer" class="max-h-52 overflow-y-auto custom-scrollbar border border-zinc-800 rounded-2xl p-2 space-y-1 bg-zinc-950/80">
                ${(state.tools || []).map(t => {
                  const isChecked = applicable.includes(t.id);
                  return `
                    <label class="coupon-tool-item flex items-center justify-between p-2 hover:bg-zinc-900/80 rounded-xl text-xs cursor-pointer transition-colors" data-name="${(t.name || '').toLowerCase()}">
                      <div class="flex items-center gap-2.5 min-w-0">
                        <input type="checkbox" value="${t.id}" ${isChecked ? 'checked' : ''} onchange="window.onCouponToolCheckChange()" class="coupon-tool-checkbox rounded border-zinc-700 text-amber-500 focus:ring-0">
                        <span class="text-zinc-200 font-medium truncate">${t.name}</span>
                      </div>
                      <span class="text-[10px] text-zinc-500 px-2 py-0.5 rounded-md bg-zinc-900 border border-zinc-800 shrink-0">${t.category_name || 'Tool'}</span>
                    </label>
                  `;
                }).join('')}
              </div>
            ...
            <label class="flex items-center gap-3 p-3 rounded-2xl bg-zinc-900/40 border border-zinc-800 cursor-pointer">
              <input type="checkbox" id="couponActiveCheck" ${cp.is_active !== false ? 'checked' : ''} class="rounded border-zinc-700 text-emerald-500 focus:ring-0">
              <div class="text-xs">
                <span class="font-semibold text-zinc-200">Coupon is Active & Ready</span>
                <p class="text-[10px] text-zinc-500">Active coupons can be redeemed by customers on the store.</p>
              </div>
            </label>
          </div>

          <div class="flex items-center justify-end gap-3 pt-4 border-t border-zinc-800">
            <button type="button" onclick="window.closeCouponModal()" class="px-5 py-2.5 rounded-xl border border-zinc-700 text-zinc-300 hover:text-white hover:bg-zinc-800 text-xs font-semibold transition-all">Cancel</button>
            <button type="button" id="btnSaveCoupon" onclick="window.saveCouponModal()" class="px-6 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-zinc-950 font-bold rounded-xl text-xs shadow-lg shadow-amber-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center gap-2">
  ...
  function renderToast() {
    ...
    return `
      <div class="fixed top-6 right-6 z-[999999] px-6 py-3.5 rounded-2xl shadow-2xl border text-xs font-bold flex items-center gap-3 transition-all animate-bounce ${
        isError ? 'bg-red-950/95 border-red-500/60 text-red-200 shadow-red-500/20' : 'bg-emerald-950/95 border-emerald-500/60 text-emerald-200 shadow-emerald-500/20'
      }">
...
${escapeHtml(item.message || '(No message content)')}
  ...
  window.setCouponScope = function (scope) {
    const section = document.getElementById('couponToolsSection');
    const optAll = document.getElementById('scopeOptionAll');
    const optSpec = document.getElementById('scopeOptionSpecific');
    
    if (section) {
      section.style.display = (scope === 'specific' ? 'block' : 'none');
    }
    if (optAll && optSpec) {
      if (scope === 'all') {
        optAll.className = 'flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all border-amber-500/60 bg-amber-500/10 text-amber-300 shadow-sm shadow-amber-500/10';
        optSpec.className = 'flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all border-zinc-800 bg-zinc-900/60 text-zinc-400 hover:border-zinc-700';
      } else {
        optSpec.className = 'flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all border-amber-500/60 bg-amber-500/10 text-amber-300 shadow-sm shadow-amber-500/10';
        optAll.className = 'flex items-center gap-3 p-3 rounded-2xl border cursor-pointer transition-all border-zinc-800 bg-zinc-900/60 text-zinc-400 hover:border-zinc-700';
      }
    }
    if (state.editingCoupon) {
      state.editingCoupon.scope = scope;
    }
  };

  window.onCouponToolCheckChange = function () {
    const checked = document.querySelectorAll('.coupon-tool-checkbox:checked');
    const countEl = document.getElementById('couponSelectedToolsCount');
    if (countEl) countEl.innerText = checked.length;
    if (state.editingCoupon) {
      state.editingCoupon.applicable_tools = Array.from(checked).map(cb => cb.value);
    }
  };

  window.couponSelectAllTools = function (select) {
    const checkboxes = document.querySelectorAll('.coupon-tool-checkbox');
    checkboxes.forEach(cb => {
      const item = cb.closest('.coupon-tool-item');
      if (!item || item.style.display !== 'none') {
        cb.checked = select;
      }
    });
    window.onCouponToolCheckChange();
  };

  window.filterCouponToolsList = function (query) {
    const q = (query || '').toLowerCase().trim();
    const items = document.querySelectorAll('.coupon-tool-item');
    items.forEach(item => {
  ...
  window.saveCouponModal = async function () {
    ...
    const discountEl = document.getElementById('couponDiscountInput');
    const activeEl = document.getElementById('couponActiveCheck');
    const scopeRadio = document.querySelector('input[name="couponScopeRadio"]:checked');
    const btn = document.getElementById('btnSaveCoupon');

    const code = (codeEl ? codeEl.value : (cp.code || '')).trim().toUpperCase();
    const discount_value = Number(discountEl ? discountEl.value : (cp.discount_value || 20));
    const scope = (scopeRadio ? scopeRadio.value : (cp.scope || 'all'));
    const is_active = (activeEl ? activeEl.checked : (cp.is_active !== false));

    const checkedBoxes = document.querySelectorAll('.coupon-tool-checkbox:checked');
    const applicable_tools = scope === 'specific' ? Array.from(checkedBoxes).map(cb => cb.value) : [];
    ...
    if (scope === 'specific' && applicable_tools.length === 0) {
      showToast('Please select at least 1 tool, or choose All Tools', 'error');
      return;
    }
    ...
    try {
      let result;
      if (cp.id) {
        result = await apiFetch(`/coupons/${cp.id}`, { method: 'PUT', body: JSON.stringify(payload) });
      } else {
        result = await apiFetch('/coupons', { method: 'POST', body: JSON.stringify(payload) });
      }

      if (!result || result.success === false) {
        const msg = (result && result.message && result.message.includes('duplicate'))
          ? 'This coupon code already exists! Use a different code.'
          : ((result && result.message) || 'Failed to save coupon');
        showToast(msg, 'error');
        if (btn) {
          btn.disabled = false;
          btn.innerHTML = '<span>Save Coupon</span>';
        }
        return;
      }
      ...
      if (cp.id) {
        ...
        if (idx !== -1) {
          state.coupons[idx] = { ...state.coupons[idx], ...payload, id: cp.id };
        }
      } else {
        const saved = result.coupon || { ...payload, id: 'cp_' + Date.now() };
        state.coupons.unshift(saved);
      }
      ...
      showToast('Failed to save coupon — please check connection', 'error');
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<span>Save Coupon</span>';
      }
